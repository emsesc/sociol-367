...
```         
Scenarios in which the research goals include the identification of specific topics, sentiment analysis, or more generally, detecting opinions or emotions around an issue are best for considering use of dictionary-based approaches. The scenario should also have a relevant and high-quality dictionary for best practice. Depending on what the goals of the research study is, using dictionary-based approaches affect the outcome variables of the research. For example, the research questions would now be centered around the categories that the dictionary-based approach identifies. If the analysis yields a valence score, the research question would be shaped around this spectrum.
```
...
By the results, the average sentiment for "night" is closer to 1 (positive sentiment) than the average sentiment for "day". This would imply that tweets tend to be more positive when posted at night.
```
...
```         
My dictionary is intended to capture the level of "unity" or "togetherness" a tweet has. A tweet with a score of 0 is divisive, and a score of 1 is uniting. The graph shows that the tweets tend to have a valence closer to 1 during night time, which implies that tweets tend to be more divisive during the day.
```
