...
# The x argument of the mean() function is the R object containing the numbers that you want to find the mean of. The trim argument defines the fraction of numbers to be removed from the x argument. The na.rm argument is a true/false value that defines whether NA values should be counted.
```
...
# the key difference is that single brackets return a data frame with one column, while double brackets return the actual vector (column)
```
...
```         
1) I've worked with R before, but I mostly relied on ChatGPT to write code, so I want to become more familiar with the syntax this semester.
2) I want to learn how to properly conduct my own statistically sound research and go through the steps of data collection, cleaning, and analysis.
3) I want to learn how to apply ethical principles to evaluate my own research.
```
