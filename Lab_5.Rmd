...
Network Representations
  edge list: List of edges that shows how 
...
Types of networks
  directed vs undirected network: A directed network has arrows that represent a direction of connection between nodes, while an undirected network has edges without direction.
...
senate_net <- common_subcomm %>% 
  ...
  filter(common_subcomm >= 3) %>% 
  graph_from_data_frame(vertices=senators)

V(senate_net)$color_party<-ifelse(V(senate_net)$gender=="F", "green", "purple")

ggraph(senate_net, layout="kk") +
	geom_node_point(color=V(senate_net)$color_party) +
	geom_edge_link(width=.05) +
	theme_void() +
	geom_node_text(aes(label=full_name), size=2)
```
...
centrality_score <- as.data.frame(betweenness(senate_net)) %>%
  ...
  inner_join(senators, by = "bioguide_id")

colnames(centrality_score)[2] <- "score"
...
average_shortest_path_length <- average.path.length(senate_net)

reprsentatives <- (congress %>% filter(type=='rep'))
rep_net <- common_subcomm %>% 
  filter((to %in% representatives$bioguide_id) & (from %in% representatives$bioguide_id)) %>% 
  filter(common_subcomm >= 3) %>% 
  graph_from_data_frame(vertices=representatives)

average_shortest_path_length <- average.path.length(rep_net)
```
